...
public class BleWrapper {
    ...
    private static final int RSSI_UPDATE_TIME_INTERVAL = 500; // 1.5 seconds

    /* callback object through which we are returning results to the caller */
    private in.optimux.www.newremote.BleWrapperUiCallbacks mUiCallback = null;
    /* define NULL object for UI callbacks */
    private static final in.optimux.www.newremote.BleWrapperUiCallbacks NULL_CALLBACK = new in.optimux.www.newremote.BleWrapperUiCallbacks.Null();
    
    /* creates BleWrapper object, set its parent activity and callback object */
    public BleWrapper(Activity parent, in.optimux.www.newremote.BleWrapperUiCallbacks callback) {
    	this.mParent = parent;
    ...
    public void getCharacteristicValue(BluetoothGattCharacteristic ch) {
        ...
        if(uuid.equals(in.optimux.www.newremote.BleDefinedUUIDs.Characteristic.HEART_RATE_MEASUREMENT)) { // heart rate
        ...
        else if (uuid.equals(in.optimux.www.newremote.BleDefinedUUIDs.Characteristic.HEART_RATE_MEASUREMENT) || // manufacturer name string
        		 uuid.equals(in.optimux.www.newremote.BleDefinedUUIDs.Characteristic.MODEL_NUMBER_STRING) || // model number string)
        		 uuid.equals(in.optimux.www.newremote.BleDefinedUUIDs.Characteristic.FIRMWARE_REVISION_STRING)) // firmware revision string
        {
        ...
        else if(uuid.equals(in.optimux.www.newremote.BleDefinedUUIDs.Characteristic.APPEARANCE)) { // appearance
        	...
        	intValue += rawValue[0];
        	strValue = in.optimux.www.newremote.BleNamesResolver.resolveAppearance(intValue);
        }
        else if(uuid.equals(in.optimux.www.newremote.BleDefinedUUIDs.Characteristic.BODY_SENSOR_LOCATION)) { // body sensor location
        	...
        	intValue = rawValue[0];
        	strValue = in.optimux.www.newremote.BleNamesResolver.resolveHeartRateSensorLocation(intValue);
        }
        else if(uuid.equals(in.optimux.www.newremote.BleDefinedUUIDs.Characteristic.BATTERY_LEVEL)) { // battery level
...
//                }
        ...
        public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        	...
        	String serviceName = in.optimux.www.newremote.BleNamesResolver.resolveServiceName(characteristic.getService().getUuid().toString().toLowerCase(Locale.getDefault()));
        	String charName = in.optimux.www.newremote.BleNamesResolver.resolveCharacteristicName(characteristic.getUuid().toString().toLowerCase(Locale.getDefault()));
